...
local panelText, titleText, okButton, itemText, item2Text
local marks, puzzleIndex, answerRect, currentQuestion, bgShine
local playerShip, shipGroup, currentPlayer
local worldIndex, levelIndex, opPanelGroup
...
local function gotoNextScreen()
	...
	transition.to(titleText, {delay = 600, transition = easing.inBack, y = display.screenOriginY - titleText.height, time=1000})
...
local function initScreenElements(group)
	...
	opPanelGroup.x = screenLeft - screenLeft
...
--	local randomQuestion = mathRandom(1, #operations)
	print( "index " .. puzzleIndex .. "")
	currentQuestion = operations[puzzleIndex]
	titleText.text = currentQuestion.text
	titleText.anchorY = 0
	titleText.x = centerX
	titleText.y = screenTop + screenTop
...
function scene:create( event )
	...
	local x = display.newText("X", screenRight - 180, centerY - 120, settings.fontName, 80)
	
	local line = display.newText("_____", screenRight - 300, centerY - 100, settings.fontName, 90)
...
local function animateScene()
	...
	transition.to(titleText, {delay = 300, transition = easing.outBounce, y = screenTop + 50, time=1000})
